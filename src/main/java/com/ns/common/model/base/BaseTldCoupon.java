package com.ns.common.model.base;

import com.jfinal.plugin.activerecord.Model;
import com.jfinal.plugin.activerecord.IBean;

/**
 * Generated by JFinal, do not modify this file.
 */
@SuppressWarnings({"serial", "unchecked"})
public abstract class BaseTldCoupon<M extends BaseTldCoupon<M>> extends Model<M> implements IBean {

	public M setID(java.lang.String ID) {
		set("ID", ID);
		return (M)this;
	}
	
	public java.lang.String getID() {
		return getStr("ID");
	}

	public M setNAME(java.lang.String NAME) {
		set("NAME", NAME);
		return (M)this;
	}
	
	public java.lang.String getNAME() {
		return getStr("NAME");
	}

	public M setDESCRIPTION(java.lang.String DESCRIPTION) {
		set("DESCRIPTION", DESCRIPTION);
		return (M)this;
	}
	
	public java.lang.String getDESCRIPTION() {
		return getStr("DESCRIPTION");
	}

	public M setSafetyAmount(java.math.BigDecimal safetyAmount) {
		set("SAFETY_AMOUNT", safetyAmount);
		return (M)this;
	}
	
	public java.math.BigDecimal getSafetyAmount() {
		return get("SAFETY_AMOUNT");
	}

	public M setDiscountAmount(java.math.BigDecimal discountAmount) {
		set("DISCOUNT_AMOUNT", discountAmount);
		return (M)this;
	}
	
	public java.math.BigDecimal getDiscountAmount() {
		return get("DISCOUNT_AMOUNT");
	}

	public M setTotalNumber(java.lang.Integer totalNumber) {
		set("TOTAL_NUMBER", totalNumber);
		return (M)this;
	}
	
	public java.lang.Integer getTotalNumber() {
		return getInt("TOTAL_NUMBER");
	}

	public M setRemainNumber(java.lang.Integer remainNumber) {
		set("REMAIN_NUMBER", remainNumber);
		return (M)this;
	}
	
	public java.lang.Integer getRemainNumber() {
		return getInt("REMAIN_NUMBER");
	}

	public M setCouponType(java.lang.Integer couponType) {
		set("COUPON_TYPE", couponType);
		return (M)this;
	}
	
	public java.lang.Integer getCouponType() {
		return getInt("COUPON_TYPE");
	}

	public M setImageUrl(java.lang.String imageUrl) {
		set("IMAGE_URL", imageUrl);
		return (M)this;
	}
	
	public java.lang.String getImageUrl() {
		return getStr("IMAGE_URL");
	}

	public M setStartDt(java.lang.String startDt) {
		set("START_DT", startDt);
		return (M)this;
	}
	
	public java.lang.String getStartDt() {
		return getStr("START_DT");
	}

	public M setEndDt(java.lang.String endDt) {
		set("END_DT", endDt);
		return (M)this;
	}
	
	public java.lang.String getEndDt() {
		return getStr("END_DT");
	}

	public M setENABLED(java.lang.Integer ENABLED) {
		set("ENABLED", ENABLED);
		return (M)this;
	}
	
	public java.lang.Integer getENABLED() {
		return getInt("ENABLED");
	}

	public M setVERSION(java.lang.Integer VERSION) {
		set("VERSION", VERSION);
		return (M)this;
	}
	
	public java.lang.Integer getVERSION() {
		return getInt("VERSION");
	}

	public M setSTATUS(java.lang.Integer STATUS) {
		set("STATUS", STATUS);
		return (M)this;
	}
	
	public java.lang.Integer getSTATUS() {
		return getInt("STATUS");
	}

	public M setREMARK(java.lang.String REMARK) {
		set("REMARK", REMARK);
		return (M)this;
	}
	
	public java.lang.String getREMARK() {
		return getStr("REMARK");
	}

	public M setCreateBy(java.lang.String createBy) {
		set("CREATE_BY", createBy);
		return (M)this;
	}
	
	public java.lang.String getCreateBy() {
		return getStr("CREATE_BY");
	}

	public M setCreateDt(java.lang.String createDt) {
		set("CREATE_DT", createDt);
		return (M)this;
	}
	
	public java.lang.String getCreateDt() {
		return getStr("CREATE_DT");
	}

	public M setUpdateDt(java.lang.String updateDt) {
		set("UPDATE_DT", updateDt);
		return (M)this;
	}
	
	public java.lang.String getUpdateDt() {
		return getStr("UPDATE_DT");
	}

}
